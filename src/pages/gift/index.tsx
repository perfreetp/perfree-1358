...
const GiftPage: React.FC = () => {
  ...
  const [stagedCombos, setStagedCombos] = useState<StagedCombo[]>([]);
  const [showStagedView, setShowStagedView] = useState(false);
  const [showExportModal, setShowExportModal] = useState(false);
  const [exportComboId, setExportComboId] = useState('');
  ...
  const handleStageCombo = (combo: GiftCombo) => {
    if (stagedCombos.some(s => s.combo.id === combo.id)) {
      Taro.showToast({ title: '已暂存该方案', icon: 'none' });
      return;
    }
    if (stagedCombos.length >= 3) {
      Taro.showToast({ title: '最多暂存3套方案', icon: 'none' });
      return;
    }
    setStagedCombos(prev => [...prev, { combo, stagedAt: Date.now() }]);
    Taro.showToast({ title: '已加入暂存', icon: 'success' });
  };

  const handleRemoveStaged = (comboId: string) => {
    setStagedCombos(prev => prev.filter(s => s.combo.id !== comboId));
  };

  const getGiftSummary = (combo: GiftCombo): string => {
    const recipientName = getOptionName('recipient', selectedRecipient);
    const occasionName = getOptionName('occasion', selectedOccasion);
    const peopleAnalysis = getComboPeopleAnalysis(combo);
    const topPeople = peopleAnalysis.slice(0, 2).map(p => p.name).join('、');
    const parts: string[] = [];
    if (recipientName) parts.push(`特别适合送给${recipientName}`);
    if (occasionName) parts.push(`${occasionName}场景首选`);
    if (topPeople) parts.push(`人群偏向${topPeople}`);
    if (parts.length === 0) {
      return `${combo.name}：${combo.wines.length}款精选搭配，合计¥${combo.total}`;
    }
    return `${combo.name}：${parts.join('，')}，合计¥${combo.total}`;
  };

  const generateComboShareText = (combo: GiftCombo) => {
    const recipientName = getOptionName('recipient', selectedRecipient);
    const occasionName = getOptionName('occasion', selectedOccasion);
    const total = combo.total;
    
    const lines = combo.wines.map((w, i) => {
    ...
    if (recipientName) contextParts.push(`送给${recipientName}`);
    if (occasionName) contextParts.push(occasionName);
    const contextLine = contextParts.length > 0 ? `\n🎯 ${contextParts.join(' · ')}` : '';

    const summary = getGiftSummary(combo);
    
    const content = `🎁 【成套推荐】送礼方案\n${contextLine}\n\n${lines}\n\n━━━━━━━━━━━━━\n💡 ${summary}\n共${combo.wines.length}款，合计 ¥${total}\n\n—— 来自「酒识百科」`;
    return {
      title: `送礼方案：${combo.name}，合计¥${total}`,
      content
    };
  };

  const handleExportCombo = (comboId: string) => {
    setExportComboId(comboId);
    setShowExportModal(true);
  };

  const getExportCombo = (): GiftCombo | undefined => {
    return combos.find(c => c.id === exportComboId);
  };

  const handleCopyText = () => {
    const combo = getExportCombo();
    if (!combo) return;
    const { content } = generateComboShareText(combo);
  ...
  const renderResult = () => {
    ...
    const exportCombo = getExportCombo();
      ...
      <View>
        ...
        {combos.length > 0 ? (
          combos.map((combo, comboIndex) => {
            ...
            const isStaged = stagedCombos.some(s => s.combo.id === combo.id);
            return (
              <View key={combo.id} className={styles.comboCard}>
                ...
                <View className={styles.comboSummary}>
                  <Text className={styles.summaryIcon}>💡</Text>
                  <Text className={styles.summaryText}>{getGiftSummary(combo)}</Text>
                </View>
                  ...
                  {combo.wines.map((wine, wineIndex) => (
                    <View key={`${combo.id}-${wine.id}`} className={styles.comboWineItem}>
                      ...
                      <View className={styles.winePriceWrap}>
                        ...
                        <View className={styles.wineActions}>
                          <Button
                            className={classnames(styles.wineActionBtn, isInCompare(wine.id) && styles.wineActionActive)}
                            onClick={() => handleToggleCompare(wine.id)}
                          >
                            {isInCompare(wine.id) ? '✓ 对比' : '+ 对比'}
                          </Button>
                          <Button
                ...
                <View className={styles.comboFooter}>
                  ...
                  <View className={styles.comboActions}>
                    <Button
                      className={classnames(styles.actionBtn, styles.stageBtn, isStaged && styles.stagedBtn)}
                      onClick={() => handleStageCombo(combo)}
                    >
                      {isStaged ? '📌 已暂存' : '📌 暂存'}
                    </Button>
                    <Button
                    ...
                    <Button
                      className={classnames(styles.actionBtn, styles.shareBtn)}
                      onClick={() => handleExportCombo(combo.id)}
                    >
                      📤
                    </Button>
                  </View>
        ...
        {stagedCombos.length > 0 && (
          <View className={styles.stagedSection}>
            <View className={styles.stagedHeader}>
              <Text className={styles.stagedTitle}>📋 备选方案对比（{stagedCombos.length}/3）</Text>
              <Button className={styles.stagedToggle} onClick={() => setShowStagedView(!showStagedView)}>
                {showStagedView ? '收起' : '展开'}
              </Button>
            </View>

            {showStagedView && (
              <View className={styles.stagedList}>
                {stagedCombos.map((staged) => {
                  const pa = getComboPeopleAnalysis(staged.combo);
                  const oa = getComboOccasionAnalysis(staged.combo);
                  return (
                    <View key={staged.combo.id} className={styles.stagedCard}>
                      <View className={styles.stagedCardHeader}>
                        <Text className={styles.stagedCardBadge}>{staged.combo.badge}</Text>
                        <Text className={styles.stagedCardName}>{staged.combo.name}</Text>
                        <Button className={styles.stagedRemoveBtn} onClick={() => handleRemoveStaged(staged.combo.id)}>✕</Button>
                      </View>
                      <View className={styles.stagedCardBody}>
                        <View className={styles.stagedWineList}>
                          {staged.combo.wines.map(w => (
                            <Text key={w.id} className={styles.stagedWineName}>{w.name} ¥{w.price}</Text>
                          ))}
                        </View>
                        <View className={styles.stagedInfo}>
                          <Text className={styles.stagedTotal}>合计 ¥{staged.combo.total}</Text>
                          <View className={styles.stagedTags}>
                            {pa.map(p => (
                              <Text key={p.name} className={styles.stagedTag}>👥 {p.name} {p.percent}%</Text>
                            ))}
                            {oa.map(o => (
                              <Text key={o.name} className={styles.stagedTag}>🎉 {o.name} {o.percent}%</Text>
                            ))}
                          </View>
                        </View>
                      </View>
                      <View className={styles.stagedCardActions}>
                        <Button className={styles.stagedActionBtn} onClick={() => handleAddComboToFavorites(staged.combo)}>❤️ 收藏</Button>
                        <Button className={styles.stagedActionBtn} onClick={() => handleAddComboToCompare(staged.combo)}>📊 对比</Button>
                        <Button className={styles.stagedActionBtn} onClick={() => handleExportCombo(staged.combo.id)}>� 分享</Button>
                      </View>
                    </View>
                  );
                })}
        ...
        {showExportModal && exportCombo && (
          <View className={styles.exportModal} onClick={() => setShowExportModal(false)}>
            <View className={styles.exportContent} onClick={(e) => e.stopPropagation()}>
              ...
              <Text className={styles.exportSubtitle}>{getGiftSummary(exportCombo)}</Text>
              ...
              <View className={styles.exportOptions}>
                <Button className={styles.exportOption} onClick={handleCopyText}>
                  ...
                  <View className={styles.exportOptionInfo}>
                    ...
                    <Text className={styles.exportOptionDesc}>复制方案文字到剪贴板</Text>
                  </View>
                </Button>
                ...
                <Button className={styles.exportOption} onClick={handleShowCard}>
                  ...
                  <View className={styles.exportOptionInfo}>
                    <Text className={styles.exportOptionTitle}>方案卡片</Text>
                    <Text className={styles.exportOptionDesc}>生成精美方案图片卡片</Text>
                  </View>
                </Button>
              </View>
            ...
            <ScrollView className={styles.cardScroll} onClick={(e) => e.stopPropagation()}>
              <View className={styles.giftCard}>
                <View className={styles.giftCardHeader}>
                  ...
                  <Text className={styles.giftCardTitle}>🍷 {exportCombo.name}</Text>
                  <Text className={styles.giftCardSubtitle}>来自「酒识百科」</Text>
                </View>

                <View className={styles.giftCardContext}>
                  {getOptionName('recipient', selectedRecipient) && (
                    <Text className={styles.giftCardContextTag}>
                      🎯 送给{getOptionName('recipient', selectedRecipient)}
                    </Text>
                  )}
                  {getOptionName('occasion', selectedOccasion) && (
                    <Text className={styles.giftCardContextTag}>
                      🎉 {getOptionName('occasion', selectedOccasion)}
                    </Text>
                  )}
                ...
                <View className={styles.giftCardBody}>
                  {exportCombo.wines.map((wine, index) => (
                ...
                <View className={styles.giftCardSummary}>
                  ...
                  <Text className={styles.giftCardSummaryText}>{getGiftSummary(exportCombo)}</Text>
                </View>

                <View className={styles.giftCardFooter}>
                  <View className={styles.giftCardTotal}>
                    <Text className={styles.giftCardTotalLabel}>共{exportCombo.wines.length}款 · 合计</Text>
                    <Text className={styles.giftCardTotalPrice}>¥{exportCombo.total}</Text>
                  </View>
